...
function ajaxHeart(e) {
  ...
  .then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); //  form tag has a name attribute of 'heart'
    (0, _bling.$)('.heart-count').textContent = res.data.hearts.length; // new length of hearts property of new, response data data
    if (isHearted) {
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (place) {
      ...
      bounds.extend(position); // keep extending bounds for eveery marker
      var marker = new google.maps.Marker({ map: map, position: position });
      marker.place = place;
      return marker;
    });

    // when someone clicks on a marker, show details of the place
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        // addListener is addEventListener of google maps
        var html = '\n          <div class="popup">\n            <a href="/store/' + this.place.slug + '">\n              <img src="/uploads/' + (this.place.photo || 'store.png') + '" alt="' + this.place.name + '" />\n              <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n            </a>\n          </div>\n        ';
        infoWindow.setContent(html);
        infoWindow.open(map, this);
      });
    });
    // zoom map to fit all markets perfectly
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    // use axios to hit our endpoints
  ...
  searchInput.on('keyup', function (e) {
    // if they aren't pressing up, down, or enter, then who cares!
...
var heartForms = (0, _bling.$$)('form.heart'); // every form with a class of heart
